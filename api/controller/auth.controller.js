...
try {
   ...
   const token = jwt.sign({id : validUser._id},process.env.JWT_SECRET);//secret key to make the token unique
const { password: password , ...User} = validUser._doc;//hiding password
res
.cookie('access_token', token, {
   httpOnly: true})
   .status(200)
   .json(rest);// scret key to make the token unique
